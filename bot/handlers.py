...
async def max_date(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        ...
        if user_data.get("is_editing") and "appointment_id" in user_data:
            action_text, appt_id = db.update_appointment(telegram_user_id, user_data["appointment_id"], user_data)
        else:
            action_text, appt_id = db.save_appointment(telegram_user_id, user_id, user_data)
        
        user_data["appointment_id"] = appt_id
        ...
        vps_success = vps.create_vps_config(user_data) # passes user_data which has appointment_id
        if not vps_success:
            await update.message.reply_text(
        ...
        appointment_id = context.user_data.get("appointment_id")
        schedule_ids, error_detail = vps.discover_schedule_ids(appt_email, appointment_id)
...
async def schedule_select_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if db.check_existing_schedule_id(schedule_id):
        return await show_main_menu(update, context, text="❌ Este Schedule ID ya se encuentra registrado. No se puede duplicar.")

    # Phase 3: Update config with selected SCHEDULE_ID and start PM2
    appointment_id = context.user_data.get("appointment_id")
    success = vps.set_schedule_id_and_start(appt_email, schedule_id, appointment_id)
...
async def manual_schedule_id(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if db.check_existing_schedule_id(schedule_id):
        await update.message.reply_text("❌ Este Schedule ID ya se encuentra registrado. No se puede duplicar.")
        return await show_main_menu(update, context)
